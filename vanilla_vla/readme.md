...
- **출발 정체**: 정지 프레임만 보면 0 을 낸다(시연 첫 프레임이 정지). 하네스 kick 으로 푼다.
- **D1 공유공간(T1)**: 시연이 45편뿐이라 stop-go 로 10 m 쯤에서 멎는다. `<think>` 가 "sidewalk … signal red" 라고
  쓰는 것을 보면 면을 보도로 착각한다. 조향 방법이 고칠 수 있는 종류의 문제가 아니다.
- **T2 바리케이드**: 공사 구역 앞에서 우회하지 못하고 밀고 선다.
- 신호등 카운트다운 숫자는 384×384 에서 읽히지 않는다. 신호 준수는 색·손바닥 심볼에 의존한다.
